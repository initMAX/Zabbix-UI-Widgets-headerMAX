...
use Modules\Header\Actions\Widgets\HeaderForm;
...
class CWidgetConfig
{
	...
	public static function getKnownWidgetTypes(string $context): array {
		$types = [
			...
			HeaderForm::WIDGET_TYPE 	=> _('Header')
		];
	...
	public static function getJSClasses(): array {
		return [
			...
			HeaderForm::WIDGET_TYPE	    => 'CWidget'
		];
	}
	...
	private static function getDefaultDimensions(): array {
		return [
			...
			HeaderForm::WIDGET_TYPE	    => ['width' => 5,  'height' => 1]
		];
	}
	...
	public static function getForm(string $type, string $data, ?string $templateid): CWidgetForm {
		switch ($type) {
			...
			case HeaderForm::WIDGET_TYPE:
				return new HeaderForm($data, $templateid);
