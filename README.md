...
<img src="./.readme/badge/version.svg" alt="version 2.7.2">
...
Everything lives in one familiar widget form - pick **text or HTML** and choose font, size, colours and alignment. Nothing is hidden in FREE: every PRO-only control - the one-click Example presets, the **Generate with AI** button and each PRO field - stays visible inline, greyed out with a padlock whose tooltip explains it and links to the upgrade.

<div align="center">
<img src="./.readme/screen/06-settings.png" width="440" alt="headerMAX configuration panel">
</div>

## Generate the panel with AI (PRO)

Under the Content box there is a **Generate with AI** row. Describe what the panel should show - "CPU utilisation of Zabbix server as a gauge" - and the answer streams into a preview with a live rendering of the panel underneath it. Nothing touches the Content field until you press **Use this content**; **Regenerate** asks what should be different and hands the answer back to the model as the starting point.

You can attach a **layout image** - pick a file, paste a screenshot, or drop one on the panel - and the model follows its arrangement and colours. The numbers still come from your own Zabbix data: every live value is an expression macro, never something read off the picture.

<div align="center">
<img src="./.readme/screen/07-ai-generate.png" width="620" alt="Generating a panel with AI - instruction, preview and the rendered result">
</div>

The provider (OpenAI, Anthropic, Gemini or any OpenAI-compatible endpoint), its model, tuning, an optional MCP server and a connection test live in **Administration → AI General → headerMAX** - the same shared section the other initMAX AI products use. FREE sees that page too, padlocked: it says what PRO configures there rather than answering the menu row with an error.

<div align="center">
<img src="./.readme/screen/08-ai-settings.png" width="620" alt="Administration - AI General - headerMAX">
</div>
...
| **Generate the panel content with AI** (OpenAI · Anthropic · Gemini · custom endpoint) |   ❌   |   ✅   |
| **Streamed answer with accept / discard** - the Content field is never overwritten unasked |   ❌   |   ✅   |
| **Administration → AI General → headerMAX** (provider, model, tuning, connection test) |   ❌   |   ✅   |
